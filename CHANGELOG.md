...
`label_ids` did, added for `ThreadModal`'s "Labels: …" line and
`LabelPickerModal`'s pre-check), and the Email list itself had nowhere to
render them once it did.

`gauth._thread_summary` now unions `labelIds` across every message in the
thread (same pattern as its existing `unread` check) and stores it on the
row as `"labelIds"`. `list_threads`'s cache-reuse path passes cached rows
through unchanged, so a row fetched before this change simply has no
`labelIds` until it's next refetched (Gmail bumps historyId on label edits,
so that happens naturally) — no cache migration needed.

New `_thread_label_chips` (`main.py`) turns a thread's `labelIds` into a
...
list picks up the change on its next natural refresh (same as any other
out-of-modal thread mutation), consistent with existing behavior.
...
**Provider-aware Hermes Ask, reachable from anywhere (`Ctrl+K`).** The
Dashboard's Hermes card always said "HERMES ASK" even when `Settings.
ai_provider` was set to Claude Code/opencode/Gemini CLI — fixed: the card's
title (`#hermes-pane-title`, now a stable id via `_pane_title_row`'s new
`text_id` param) and Input placeholder are generated by a new `GoogleTUI.
_hermes_ask_title()`/`_update_hermes_labels()` pair (`ask.display_name(pid)`,
new in `ask.py` — the class attribute alone, no instantiation), called at
startup and live from `on_radio_set_changed`'s AI-provider branch. New
`HermesAskModal` (`Ctrl+K`, `bindings.ActionSpec("hermes_popup")`) pops up a
quick-ask box from ANY tab — same title/placeholder logic, same submit path
(`GoogleTUI._hermes_submit`/`_hermes_thread`, both now parameterized on the
target `RichLog` instead of hardcoding `#hermes-log`, so the Dashboard card
and the popup share one LLM-calling implementation, not two). A fresh,
unshared conversation each time it opens; Esc closes. Naturally dead while
another modal is on top (Textual's binding-chain truncation at a modal
boundary — the same mechanism documented for ThreadModal's own `r`/`a`/`f`).

**Settings → Dashboard: enable/disable cards.** New `Settings.
dashboard_panes_enabled: list[str]` (`settings.py`, default all five —
matches pre-toggle behavior) + a new Settings sub-tab
(`settings-tab-dashboard`, `SelectionList#settings-dashboard-panes`,
live-applied via `on_selection_list_selected_changed`, no Save button). At
least one card always stays enabled — unchecking the last one re-selects
"Hermes Ask" with a warning notify rather than leaving an empty Dashboard.
This is deliberately the first step toward the ROADMAP's "library of
dashboard panes" framing: `DASH_PANE_IDS` is now documented as the library
itself, and adding a future external card (weather/stocks/etc.) is a
`Container` in `compose()` + one id in three existing lists, not new
plumbing.

Enabling this required a real rework, not just a checkbox: `GoogleTUI.
_dash_active` changed from a positional INDEX into `DASH_PANE_IDS` to the
active card's id STRING directly, since indexing stops being meaningful once
the visible set can be a filtered subset. Every pane-switching path
(`_focus_dash_pane`, `_goto_pane`, the new `_cycle_dash_pane`, `_adjacent`)
now takes/compares ids, never positions. This also fixed a real, already-
shipped bug found along the way: `_goto_pane`'s old `idx - 1` arithmetic was
only correct for the original 3-card Dashboard — once `[2026-07-17]` grew it
to 5 cards, `Alt+4` silently jumped to "dash-mail" instead of "hermes". New
`_apply_dashboard_panes_enabled()` (startup + every Settings checklist
change) recomputes `self._dash_enabled_ids` and toggles a `.dash-pane-
disabled` CSS class (`display: none`) on each disabled card; `_adjacent()`
walks `DASH_ADJACENCY`'s fixed grid map past a disabled card instead of
landing on it; `_cycle_dash_pane` (replacing inline arithmetic in
`action_cycle`/`action_cycle_back`) walks `_dash_enabled_ids` only.

Files: `google_tui/main.py`, `google_tui/ask.py`, `google_tui/settings.py`,
`google_tui/bindings.py`. Verified via isolated, fully-mocked `run_test`
pilots: default/live provider labeling, the Alt+4 regression fix, Ctrl+K
open/ask/Esc from a non-Dashboard tab, the Settings checklist end-to-end
(persist, hide/show cards, Tab-cycle and Alt-arrow skip disabled cards,
empty-selection fallback), plus a re-run of `[2026-07-17]`'s original
Enter→modal / narrow-mode checks against the new id-based API. A first
`width: 80%; height: 70%` CSS attempt on the popup's box left a visible gap
where the screen underneath bled through (confirmed via a side-by-side
screenshot against `GeminiInputModal`, which has none) — reverted to the
bare `.pane` sizing every other modal in this app already uses.
